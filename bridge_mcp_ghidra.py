...
def search_bookmarks(search_text: str, max_results: int = 100) -> str:
    ...
    return "\n".join(
        safe_get(
            "search_bookmarks", {"searchText": search_text, "maxResults": max_results}
        )
    )
...
def get_call_graph(function_address: str, depth: int = 1) -> str:
    ...
    return "\n".join(
        safe_get(
            "get_call_graph", {"functionAddress": function_address, "depth": depth}
        )
    )
...
def get_function_callers(function_address: str) -> str:
    ...
    """
    return "\n".join(safe_get("get_callers", {"functionAddress": function_address}))
...
def get_function_callees(function_address: str) -> str:
    ...
    """
    return "\n".join(safe_get("get_callees", {"functionAddress": function_address}))
...
@mcp.tool()
def find_constants_in_range(
    min_value: str, max_value: str, max_results: int = 500
) -> str:
    """
    ...
    return "\n".join(
        safe_get(
            ...
            {"minValue": min_value, "maxValue": max_value, "maxResults": max_results},
        )
    )
...
def list_function_calls(function_address: str) -> str:
    ...
    """
    return "\n".join(
        safe_get("list_function_calls", {"functionAddress": function_address})
    )
...
def trace_data_flow_backward(address: str) -> str:
    ...
    """
    return "\n".join(safe_get("trace_data_flow_backward", {"address": address}))
...
def trace_data_flow_forward(address: str) -> str:
    ...
    """
    return "\n".join(safe_get("trace_data_flow_forward", {"address": address}))
...
def get_function_count(filter_default_names: bool = True) -> str:
    ...
    """
    return "\n".join(
        safe_get("functions/get_count", {"filterDefaultNames": filter_default_names})
    )


@mcp.tool()
def get_functions_by_similarity(
    search_string: str,
    start_index: int = 0,
    max_count: int = 100,
...
@mcp.tool()
def get_undefined_function_candidates(
    start_index: int = 0, max_candidates: int = 100, min_reference_count: int = 1
) -> str:
    """
...
@mcp.tool()
def list_imports(library_filter: str = None, max_results: int = 500, start_index: int = 0, group_by_library: bool = True) -> str:
    """
...
@mcp.tool()
def get_strings(start_index: int = 0, max_count: int = 100, include_referencing_functions: bool = False) -> str:
    """
...
@mcp.tool()
def get_decompilation(function_name_or_address: str, offset: int = 1, limit: int = 50, 
...
@mcp.tool()
def set_decompilation_comment(function_name_or_address: str, line_number: int, comment: str, comment_type: str = "eol") -> str:
    """
